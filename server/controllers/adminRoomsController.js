  ...
  if (Array.isArray(input)) {
    return input
      .map((it) => {
        if (!it) return null;
        if (typeof it === "string")
          return { title: it.trim(), subtitle: "", iconKey: "" };
        return {
          title: (it.title || "").toString().trim(),
          subtitle: (it.subtitle || "").toString().trim(),
          iconKey: (it.iconKey || "").toString().trim(),
        };
      })
      .filter(Boolean);
  }
...
export const createAdminRoomWithImages = async (req, res) => {
  try {
    ...
    const offersArr = normalizeOffers(whatThisPlaceOffers);
...
export const updateAdminRoomWithImages = async (req, res) => {
  try {
    ...
    // ✅ update fullPrice too
    if (typeof req.body.fullPrice !== "undefined")
      room.fullPrice = Number(req.body.fullPrice) || 0;
    ...
    if (typeof req.body.whatThisPlaceOffers !== "undefined")
      room.whatThisPlaceOffers = normalizeOffers(
        req.body.whatThisPlaceOffers
      );
    ...
    if (typeof req.body.removeImages !== "undefined") {
      ...
      if (typeof req.body.removeImages === "string") {
        try {
          removeArr = JSON.parse(req.body.removeImages);
        } catch (e) {
          removeArr = req.body.removeImages
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean);
        }
      ...
      if (removeArr.length)
        room.images = (room.images || []).filter(
          (u) => !removeArr.includes(u)
        );
    }

    // handle new file uploads (req.files) — multer memory storage
