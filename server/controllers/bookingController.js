...
const createReceiptPDF = (booking, roomData, userDetails, billingName) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        .fontSize(18)
        .text("The Holiday Creators — Booking Receipt", { align: "center" });
      doc.moveDown(0.5);
      doc
        .fontSize(10)
        .fillColor("#444")
      ...
      // Booking Summary box
      doc.fillColor("#000").fontSize(12).text("Booking Summary", {
        underline: true,
      });
      doc.moveDown(0.5);
      ...
      doc.text(
        `Package: ${
          roomData && roomData.hotel && roomData.hotel.name
            ? roomData.hotel.name
            : "N/A"
        }`
      );
      doc.text(
        `Destination: ${
          roomData && roomData.hotel && roomData.hotel.address
      ...
      doc.text(
        `Package Details: ${
          roomData &&
          (roomData.roomType ||
            (roomData.room && roomData.room.roomType))
            ? roomData.roomType || roomData.room.roomType
            : "N/A"
        }`
      );
      doc.moveDown(0.5);
      ...
      // Nights (for info only – price is guest based now)
      ...
      const perNight =
        roomData && roomData.pricePerNight
          ? roomData.pricePerNight
          : booking.totalPrice || 0;
      doc.moveDown(0.5);
      doc.text(`Nights (info only): ${nights}`);
      doc.text(`Price per person: Rs ${perNight}`);
      ...
      try {
        if (booking.customization) {
          ...
          if (pkgText) {
            doc
              .fontSize(12)
              .text("Customization / Requests", { underline: true });
            doc.moveDown(0.3);
      ...
      doc.moveDown(0.5);
      doc.text(
        "If you have any questions, contact us at info@theholidaycreators.com"
      );
      doc.moveDown(1);
...
function normalizeCustomization(raw) {
  ...
  const guestsRange =
    parsed.guestsRange || parsed.guests || parsed.guests_range || null;
...
export const createBooking = async (req, res) => {
  try {
    ...
    // ✅ TOTAL PRICE ONLY BASED ON GUESTS
    const guestsCount = Number(guests) || 1;
    const totalPrice = (roomData.pricePerNight || 0) * guestsCount;

    const normalizedCustomization = normalizeCustomization(
      customization || req.body.customOptions || null
    );
...
export const getHotelBookings = async (req, res) => {
  try {
    const hotel = await Hotel.findOne({
      owner: req.auth && req.auth.userId ? req.auth.userId : null,
    });
    if (!hotel) {
...
export const createRazorpayOrder = async (req, res) => {
  try {
    ...
    if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
      console.error(
        "[RZP] Missing RAZORPAY_KEY_ID or RAZORPAY_KEY_SECRET in env"
      );
      return res.status(500).json({
        success: false,
        message:
          "Razorpay not configured on server (missing env keys). Contact admin.",
      });
    }
  ...
  try {
    ...
    const htmlBody = `
      <div style="font-family:Arial,Helvetica,sans-serif;color:#111">
        ...
        <p>Hi <strong>${
          booking.billingName || userDetails.username || "Guest"
        }</strong>,</p>
        ...
        <ul>
          ...
          <li><strong>Booking name:</strong> ${
            booking.billingName || "N/A"
          }</li>
          <li><strong>Phone number:</strong> ${
            booking.billingPhone || "N/A"
          }</li>
    ...
    if (recipients.length === 0 && booking.billingPhone)
      recipients.push(booking.billingPhone);

    const mailOptions = {
      from:
        process.env.SENDER_EMAIL ||
        process.env.SMTP_USER ||
        "no-reply@theholidaycreators.com",
      to: recipients.join(","),
      subject: `Booking Confirmed — ${hotel.name || "Your Booking"} (${
        booking._id
      })`,
      html: htmlBody,
      attachments: [
    ...
    console.error(
      "sendBookingConfirmationEmail error:",
      err && err.stack ? err.stack : err
    );
    return false;
  }
    ...
    if (bookingId) {
      ...
      if (booking) {
        ...
        sendBookingConfirmationEmail(booking).catch((e) => {
          console.error(
            "Error sending confirmation email after verify:",
            e && e.stack ? e.stack : e
          );
        });
      }
...
export const razorpayWebhookHandler = async (req, res) => {
  try {
    ...
    if (secret) {
      ...
      console.warn(
        "No RAZORPAY_WEBHOOK_SECRET set — webhook verification skipped"
      );
    }
    ...
    console.log(
      ...
      payload && payload.event ? payload.event : payload
    );

    return res.status(200).send("ok");
  } catch (err) {
    console.error(
      "razorpayWebhookHandler error:",
      err && err.stack ? err.stack : err
    );
...
export const deleteBooking = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Server error deleting booking" });
  }
};
