...
import { useClerk, useUser, UserButton } from "@clerk/clerk-react";  // ✅ add useUser
...
const Navbar = () => {
  ...
  // ✅ Get current Clerk user and role
  const { isLoaded, isSignedIn, user } = useUser();
  const role = user?.publicMetadata?.role; // "hotelOwner" or "user"
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 border-none ${isScrolled
